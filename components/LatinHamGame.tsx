...
export default function LatinHamGame({ onTriggerNewGame }: LatinHamGameProps) {
  ...
  const decodedPreset = presetParam ? decodeURIComponent(presetParam) : null
  ...
  const [showDifficultySelector, setShowDifficultySelector] = useState(!decodedPreset)
  ...
  useEffect(() => {
    ...
    setShowDifficultySelector(savedGameState ? savedGameState === 'start' : !decodedPreset)
