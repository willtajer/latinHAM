...
const LatinHamGame: React.FC = () => {
  ...
  const fetchLeaderboard = useCallback(async (): Promise<Record<string, LeaderboardEntry[]>> => {
        ...
        if (user) {
          ...
          if (savedLeaderboard) {
            try {
              ...
              console.error('Failed to parse saved leaderboard:', error)
            }
          }
        }
      }
  ...
  }, [user, isLoaded, fetchLeaderboard])
